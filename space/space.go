...
func ToMultiPart(g Geometry) (Geometry, error) {
    ...
    case GeometryCollection:
        ...
        var multiParts []Geometry
        for _, subGeom := range geom {
            multi, err := ToMultiPart(subGeom)
            if err != nil {
                return nil, err
            }
            multiParts = append(multiParts, multi)
        }
        return GeometryCollection(multiParts), nil
    default:
